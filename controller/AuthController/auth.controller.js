...
export const updateProfile = async (req, res) => {
  try {
    const { id, full_name, mobile, email, lat, lng } = req.body;
...
export const updateUserLocation = async (req, res) => {
  try {
    ...
    if (!id || !lat || !lng) {
      return res.json({
        status: "0",
        message: "id, lat and lng required",
      });
    }

    await db.query("UPDATE userdata SET lat = ?, lng = ? WHERE id = ?", [
      lat,
      lng,
      id,
    ]);
...
export const GetNotification = async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      "SELECT * FROM notifications WHERE toId = ? ORDER BY created_at DESC",
      [id],
    );
  ...
  try {
    ...
    if (type === "DRIVER") {
      const [activeRide] = await db.query(
        `SELECT id FROM booking_offer 
     WHERE driver_id = ? 
     AND status IN ('START','SCHEDULED','PICKUP')`,
        [id],
      );
      if (activeRide.length > 0) {
        return res.status(400).json({
          status: "0",
          message: "Cannot delete account while ride is in progress",
        });
      }
      await db.query(
...
export const GetSemiGoodsTypeById = async (req, res) => {
  try {
    const { good_type_id } = req.query;

    if (!good_type_id) {
      return res.status(400).json({
        status: "0",
        message: "good_type_id is required",
        result: [],
      });
    }

    const [rows] = await db.query(
      "SELECT * FROM semi_goods_type WHERE good_type_id = ?",
      [good_type_id],
    );
    ...
    console.log("Get Semi Goods type error:", error);
    return res.status(500).json({
      status: "0",
      message: "Server error",
    });
...
export const cancelRide = async (req, res) => {
  try {
    ...
    if (!offer_id) {
      return res.status(400).json({
        status: "0",
        message: "offer_id and cancel_reason are required",
        result: {},
      });
    }
...
export const SendRating = async (req, res) => {
  try {
    const { fromid, toid, offer_id, feedback, stars } = req.body;

    if (!fromid || !toid || !offer_id || !stars) {
      return res.json({
        status: "0",
    ...
    const [existing] = await db.query(
      "SELECT id FROM ratings WHERE offer_id = ?",
      [offer_id],
    );
    ...
    if (existing.length > 0) {
      return res.json({
        status: "0",
        message: "You have already submitted rating for this offer",
      });
    }

    await db.query(
      `INSERT INTO ratings 
      (from_id, to_id, offer_id, stars, feedback) 
      VALUES (?, ?, ?, ?, ?)`,
      [fromid, toid, offer_id, stars, feedback],
    );
...
export const SendOffer = async (req, res) => {
  try {
    ...
    const notifyMessage = "Driver sent you a new offer";
...
export const ChangeStatus = async (req, res) => {
  try {
    ...
    const [offer] = await db.query(
      "SELECT user_id, driver_id FROM booking_offer WHERE id = ? AND driver_id = ?",
      [offer_id, driverId],
    );
...
export const getSingleChat = async (req, res) => {
  try {
    const { user_id, user_type, other_id, other_type } = req.query;
