...
export const AddGoodsType = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      "INSERT INTO goods_type (goods_type, image) VALUES (?, ?)",
      [goods_type, image]
    );
...
export const AddSemiGoodsType = async (req, res) => {
  try {
    const { good_type_id, name } = req.body;
    ...
    await db.query(
      "INSERT INTO semi_goods_type (good_type_id, name, image) VALUES (?, ?, ?)",
      [good_type_id, name, image],
    );

    return res.status(200).json({
      status: "1",
      message: "Semi Goods type added successfully",
      result: {
        good_type_id,
        name,
        image,
    ...
    console.log("Add Semi Goods type error:", error);
    return res.status(500).json({
      status: "0",
      message: "Server error",
    });
